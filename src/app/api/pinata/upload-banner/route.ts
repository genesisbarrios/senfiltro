...
export async function POST(req: Request) {
  try {
    ...
    const filename = `${userId}_bannerImage.${ext}`;
    ...
    // HEAD-check gateway and fallback
    try {
    ...
    // try saving to user doc, unpin previous banner if present; still return upload success if DB fails
    try {
      await connectMongo();
      ...
      // unpin previous banner if present and different
      if (userDoc) {
        const prev = userDoc.bannerImage ?? userDoc.bannerPhoto ?? null;
        const prevCid = extractCid(prev);
        if (prevCid && prevCid !== hash) {
          unpinCid(prevCid).catch((e) => console.warn("unpin previous banner failed", e));
        }
        userDoc.bannerImage = hash;
        await userDoc.save();
      ...
      return NextResponse.json(
        {
          ...
          url,
          filename,
          user: userDoc ? { id: userDoc._id, walletAddress: userDoc.walletAddress, bannerPhoto: userDoc.bannerPhoto } : undefined,
        },
        { status: 200 }
      );
    } catch (dbErr) {
      console.error("upload-banner: failed to save user url", dbErr);
